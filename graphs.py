...
def graph_rd(actor_num, err_level, nominal_num, category_num):
	"""
...
def graph_nominal(actor_num, err_level, nominal_num, category_num):
    G = nx.DiGraph()
    for i in range(actor_num):
        q = normal_distribution()
...
def graph_dyadic_err(actor_num, err_level, nominal_num, category_num):
	"""
...
def graph_collective_err(actor_num, err_level, nominal_num, category_num):
	"""
...
def graph_cat(actor_num, err_level, nominal_num, category_num):
	ini_cat = np.arange(category_num).tolist()
	category = [x + 1 for x in ini_cat]
	G = nx.DiGraph()
	for i in range(actor_num):
		# c stands for category choice
		c = np.random.choice(category)
		q = normal_distribution()
		G.add_node(i, quality = q, category = c)
	pair_list = all_pairs(actor_num)
	for pair in pair_list:
		G.add_edge(pair[0], pair[1], weight = G.node[pair[1]]['quality'])
	return G

def graph_ddd(actor_num, err_level, nominal_num, category_num):
	G = nx.DiGraph()
	for i in range(actor_num):
		q = normal_distribution()
		G.add_node(i, quality = q)
